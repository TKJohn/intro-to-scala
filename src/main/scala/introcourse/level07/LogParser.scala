...
object LogParser {
  ...
  def parseIntOption(str: String): Option[Int] = str.toIntOption
  ...
  def parseLog(str: String): LogMessage = {
    ...
    val optLog: Option[LogMessage] = fields match {
      ...
      case List("E", servityStr, timestamStr, message) =>
        for {
          servity <- parseIntOption(servityStr)
          timestamp <- parseIntOption(timestamStr)
        } yield KnownLog(Error(servity), timestamp, message)
      case _ => None // Add more cases
    }
  ...
  def getErrorsOverSeverity(logs: List[LogMessage], severity: Int): List[LogMessage] =
    logs.collect { case x@KnownLog(Error(s), _, _) if s > severity => x }

  //    logs.filter(log => log match {
